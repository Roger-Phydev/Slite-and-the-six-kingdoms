...
func _process(delta: float) -> void:
	...
	if $Content.position.y < -12288: #the final section
		velocity = 0;
		skip = true;
	elif $Content.position.y < -9600: # my cat section
		velocity = 0;
		activate_birds();
	...
	elif $Content.position.y < -9500: # my cat section
		initial_timer = 0;
	elif $Content.position.y < -7904: #most important part
		velocity = 0;
		#activation of birds
		activate_birds();
		$elements/Birds.position.x -= delta*birds_speed;
		...
		if seconds-initial_timer>10: #waits for aprox 10 s
			...
			sun_animation = true;
		gradient_on_sky_color(-7904,-9600,[0.5201, 0.4712, 0.4437, 1.0],[0.2157, 0.502, 0.7373, 1.0]);
		
		# (0.2157, 0.502, 0.7373, 1.0)
	elif $Content.position.y < -4800: #When reaches people, then slow down scrolling
		velocity = 100;
		#bats animation
		activate_bats();
		$elements/Bats.position.x += delta*bat_speed; 
		#deactivating shines
		if $Content.position.y > -4919 and $Content.position.y < -4600:
			deactivate_shine(0);
			deactivate_shine(4);
			deactivate_shine(8);
			deactivate_shine(12);
		elif $Content.position.y > -5079 and $Content.position.y < -4720:
			deactivate_shine(2);
			deactivate_shine(6);
			deactivate_shine(10);
			deactivate_shine(14);
		elif $Content.position.y > -5199 and $Content.position.y < -4880:
			deactivate_shine(1);
			deactivate_shine(5);
			deactivate_shine(9);
			deactivate_shine(13);
		elif $Content.position.y > -5220 and $Content.position.y < -5000:
			deactivate_shine(3);
			deactivate_shine(7);
			deactivate_shine(11);
			deactivate_shine(15);
		gradient_on_sky_color(-4800,-7904,[0.5216, 0.4824, 0.8235, 1.0],[0.5201, 0.4712, 0.4437, 1.0]);
	else:
		gradient_on_sky_color(0,-4800,[0.0,0.0,0.0,1.0],[0.5216, 0.4824, 0.8235, 1.0])
